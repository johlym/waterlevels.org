...
- Daily archive local defaults (`DAILY_ARCHIVE_STORE=local`, short `DAILY_ARCHIVE_HOT_RETENTION_DAYS` scratch tip) are safe in `.env` for `bin/dev`. The test suite ignores `DAILY_ARCHIVE_STORE=local` unless `DAILY_ARCHIVE_ALLOW_LOCAL_IN_TEST=1`. After seeding, run `bin/rails archive:export_daily` to populate `tmp/daily_archive` for R2-style `1y` / `3y` chart reads.
