...
- **JSON API (`/api`):** first-party-only endpoints for map/hydrograph Stimulus fetches — `map/stations` (bbox, `search`, `nearest`) and `gauges/:gauge_id/observations`. Requests must send `X-WaterLevels-Client: web` plus a same-party browser context (`Sec-Fetch-Site: same-origin|same-site`, or matching `Origin`/`Referer`). Responses stay edge-cacheable (`cache_public!`) and emit `Vary: X-WaterLevels-Client, Sec-Fetch-Site`. In Cloudflare, add a Cache Rule for `/api/*` that includes those headers in the custom cache key (or enables Vary for them) so headerless scrapers cannot reuse a warm 200; Cache-Tag purge is unaffected.
