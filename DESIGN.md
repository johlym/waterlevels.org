...
- **Sync objects (`app/models/*_sync.rb`, `history_ingestion.rb`, `display_series_selection.rb`):**
  - `StationCatalogSync` (weekly / bootstrap) — discover active continuous water-body sites, filter via `Usgs::SiteTypes`, upsert series + latest, select display series, prune inactive, refresh nearby, kick a limited `NetworkStations` NLDI pass and enqueue `NetworkRefreshBatchJob` to drain the rest, warm caches.
