...
- Optional email alerts (flood category, threshold, daily digest) behind `ALERTS_ENABLED` — gauge signup, double opt-in, and `/subscriptions` manage/unsubscribe. See [`doc/alerts-contracts.md`](doc/alerts-contracts.md).
...
app/
  ...
  mailers/             contact + alert mailers (`AlertMailer` on the `notifications` queue)
...
- **Email alerts (`/subscriptions`):** `GET/POST /subscriptions` (signup or manage-link request), `GET /subscriptions/verify/:token`, `GET/PATCH /subscriptions/manage/:token` (plus pause/unpause and delete-watch), `GET/POST /subscriptions/unsubscribe/:token`. **404** when `ALERTS_ENABLED` is off (`AlertsFeatureGate`). Session-backed (`private, no-store`); not under `/alerts`.
...
- **Sync objects (`app/models/*_sync.rb`, `history_ingestion.rb`, `display_series_selection.rb`):**
  - `StationCatalogSync` (weekly / bootstrap) — discover active continuous water-body sites, filter via `Usgs::SiteTypes`, upsert series + latest, select display series, prune inactive, refresh nearby, kick a limited `NetworkStations` NLDI pass and enqueue `NetworkRefreshBatchJob` to drain the rest, warm caches. Sunday national runs checkpoint per **finished** `parameter_code` in Redis (`StationCatalogCheckpoint`, 7-day TTL, fingerprint `scope + Sunday-week + codes`). A killed/retried job skips completed codes and keeps their location ids so `prune_inactive_locations!` does not delete stations only seen in a skipped collection. Mid-parameter resume is out of scope (USGS `next` paging) — the in-flight code is re-done. After each parameter, display series are reselected only for dirty (just-upserted) locations so `/api/gauges/:id/observations` stays populated before 00060 finishes paging. Weekly location upserts refresh identity metadata (name, lat/lon, HUC, …) but **do not** overwrite `slug`, `active`, or `created_at`; series upserts **do not** overwrite `selected_for_display`. Empty hydrographs on stations that already have `has_*` / a tip log `event=hydrograph.empty`.
...
- **Jobs (`app/jobs`) + schedule (`config/sidekiq.yml` on `worker`):** catalog (Sun 03:00), latest (hourly :03), flood (hourly :30), IV tip repair catch-up (Mon–Sat hourly :35), IV scar repair catch-up (Mon–Sat hourly :50), history backfill batch (Mon–Sat every 10 minutes), NLDI network refresh batch (Mon–Sat every 10 minutes on `sync`), leftover daily drain (every 6 hours), prune (daily 09:15 UTC), alert digest (`*/15`) and quiet scan (hourly `:10`) onto `notifications`. Queues: `default` → `worker`, `sync` → `sync_worker`, `iv_repair` → `iv_repair_worker`, `iv_repair_scar` → `iv_repair_scar_worker`, `backfill` → `historical_worker`, `notifications` → `notifications_worker`.
- **Rate-limit protection:** `ApplicationJob` retries transient `Usgs::Client::Error` but **discards** USGS and NLDI `RateLimitError`s. `Usgs::RateLimitCircuit` is **per API-key id** (`tip`, `history_continuous`, `history_daily`, `history_peaks`; TTL = rest of the UTC hour) and trips on a real USGS 429. `Nldi::RateLimitCircuit` is a single unauthenticated window (`nldi:rate_limit_circuit`, same rest-of-UTC-hour TTL) tripped by NLDI `OVER_RATE_LIMIT` 429s; navigation is ~400 req/hr per client, then ~1/10s, so `NLDI_REQUEST_PAUSE_MS` defaults to 10000 and Faraday does not retry 429. `NetworkRefreshBatchJob` / `NetworkStations.refresh` skip or stop the pass when that circuit is open. Tip/catalog jobs check the tip circuit; history jobs check `Usgs::HistoryKeyPool.exhausted?` / purpose availability so one purpose trip does not stop the others (or tip sync). We do **not** keep a local used/remaining counter — USGS quota is not accurately mirrorable from our side. History backfill additionally no-ops on Sundays, honors `HistoryBackfillLock` (1h TTL, 6h cooldown), skips when the backfill queue is still busy, and is enqueued lazily from `GaugesController#show` when a station `needs_history_backfill?` (phase-1 `1y` only). The batch sizes phase-1 as absolute `HISTORY_BACKFILL_BATCH` stations (soft-capped by a theoretical 1000/hr planning ceiling) and deep `3y` up to `HISTORY_DEEP_BACKFILL_BATCH` when the daily circuit is clear. Circuit state per key is shown on `/admin`.
...
5. **No Rails session on cacheable pages.** `ApplicationController` sets `request.session_options[:skip] = true` by default so public HTML/JSON does not emit `_waterlevels_session`. Cloudflare treats `Set-Cookie` as `BYPASS` even when a Cache Rule marks the path Eligible for cache. **Contact, admin, and `/subscriptions*` opt into sessions** (`PagesController` / `ContactsController` / `Admin::*` / `SubscriptionsController` via `enable_session?`) for CSRF + flash / login / manage tokens. Gauge-page `POST /subscriptions` skips CSRF because the cached gauge HTML has no session. `csrf_meta_tags` render only when the session is enabled. `PUT /temperature_unit` skips forgery protection (preference cookie is also set client-side). **If you add another form or anything that needs CSRF/flash/session, you must opt that controller into `enable_session?` and keep its path out of the edge cache (or accept that it cannot be CDN-cached).** Do not put CSRF meta tags back on the global layout for cacheable pages. Admin and subscriptions set `private, no-store`.

**Cloudflare dashboard (ops):** public HTML/JSON need a Cache Rule with **Eligible for cache** + **Origin Cache Control: On**. Bypass `/contact*`, `/admin*`, and `/subscriptions*` (and any future session-backed or credentialed paths). Without that rule, HTML stays `DYNAMIC`/`BYPASS` and tag purge has nothing to invalidate.
...
`GET /contact` renders `Contact::FormComponent` with a Turnstile widget and is served `private, no-store`. Contact and `/subscriptions*` are the public HTML surfaces that enable the Rails session (CSRF meta tags + flash). `POST /contact` runs an `invisible_captcha` honeypot, validates a `ContactMessage`, verifies Cloudflare Turnstile (`TurnstileVerification`; bypassed in test when the secret is unset), and enqueues `ContactMailer` (delivered via bento-actionmailer in production, with premailer-rails inlining CSS). Recipient/from configured via `CONTACT_TO_EMAIL` / `MAIL_FROM`. Alert HTML mail is self-contained (see [`doc/alerts-contracts.md`](doc/alerts-contracts.md#email-rendering-as-built)).
...
- **Processes:** `Procfile` → `web` (Puma) + `worker` (Sidekiq default queue + scheduler) + `sync_worker` (`sync`, concurrency 2) + `iv_repair_worker` (`iv_repair`) + `iv_repair_scar_worker` (`iv_repair_scar`) + `historical_worker` (`backfill`) + `notifications_worker` (`notifications`, concurrency 1) + `release` (`db:migrate`); `Procfile.dev` → Rails + JS/CSS watchers. Default / historical / notifications Sidekiq processes stay concurrency 1 for 512MB eco dynos; tip IV repair uses concurrency 4 and scar IV repair concurrency 2; flood sync is one long job with an inter-state timer. Scale `notifications_worker` whenever email alerts ship (`heroku ps:scale notifications_worker=1`) — digest/quiet cron ticks land on that queue.
