...
- **`monitoring_locations`** — the central entity. **Identity:** `data_provider` (`usgs` today; reserved `usbr` / `usace` / `nwps` / `cdec` for multi-agency uplift), unique `provider_location_id` (e.g. `USGS-01646500`), unique public `site_number`, plus `agency_code` / `agency_name`. Geo columns plus **denormalized latest values** (`has_water_level/discharge/temperature`, `latest_water_level_value/unit/parameter_code`, `latest_discharge_value/unit`, `latest_temperature_c`, `latest_observed_at`, `latest_approval_status`), NWPS flood columns (`flood_stage_*`, `flood_category`, `nwps_*`), `nearby_station_ids` (jsonb), and on-stream `upstream_station_ids` / `downstream_station_ids` (jsonb, closest-first, at most 2) plus `network_synced_at`. Indexed for map (`(latitude, longitude)`), listing (`(state_code, county_name, name)`), partial flags, `flood_category`, and `data_provider`. USGS-only pipelines (IV repair, NLDI, NWPS flood enrichment) scope with `MonitoringLocation.usgs`.
