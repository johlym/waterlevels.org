...
WaterLevels.org is a public, read-mostly web app that maps USGS water-monitoring locations (gage height, streamflow/discharge, water temperature) across US states, curated Bureau of Reclamation reservoir elevations via RISE, curated U.S. Army Corps of Engineers lake/pool elevations via CWMS, curated National Weather Service NWPS gauges without a USGS site id, curated California reservoir elevations via CDEC, and enriches USGS sites with NOAA NWPS flood categories. The product surfaces:
...
- **Runtime:** Ruby 4.0.7, Rails 8.1, Puma.
...
- **HTTP:** Faraday (+ faraday-retry) for USGS/NWPS/USBR/USACE/CDEC/Turnstile.
...
app/
  ...
  models/              AR entities AND ActiveModel/PORO domain objects
    ...
    nwps/              external client + flood categories + curated no-usgsId catalog
    usbr/              RISE client + curated reservoir catalog
    usace/             CWMS client + curated project catalog
    cdec/              CDEC client + curated California reservoir catalog
...
- **Canonical gauge URL:** `/gauges/{state}/{site_number}-{slug}` (e.g. `/gauges/wa/09380000-colorado-river-at-lees-ferry`). A **numeric-only** `/gauges/{site_number}` short route 301-redirects to the canonical path (`GaugesController#ensure_canonical_path!`). Provider-prefixed ids (`usbr3514`, `usacenabraystown`, `nwpsacrw1`, `cdecoro`) have no short route — use the canonical path. State is constrained to two lowercase letters.
...
- **`monitoring_locations`** — the central entity. **Identity:** `data_provider` ∈ `DataProviders::ALL` (`usgs`, `usbr`, `usace`, `nwps`, `cdec`), unique `provider_location_id` (e.g. `USGS-01646500`, `USBR-3514`, `USACE-NAB-Raystown`, `NWPS-ACRW1`, `CDEC-ORO`), unique public `site_number`, plus `agency_code`. Geo columns plus **denormalized latest values** (`has_water_level/discharge/temperature`, `latest_water_level_value/unit/parameter_code`, `latest_discharge_value/unit`, `latest_temperature_c`, `latest_observed_at`, `latest_approval_status`), NWPS flood columns (`flood_stage_*`, `flood_category`, `nwps_*`), `nearby_station_ids` (jsonb), and on-stream `upstream_station_ids` / `downstream_station_ids` (jsonb, closest-first, at most 2) plus `network_synced_at`. Indexed for map (`(latitude, longitude)`), listing (`(state_code, county_name, name)`), partial flags, `flood_category`, and `data_provider`. USGS-only pipelines (IV repair, NLDI, USGS history backfill, NWPS flood enrichment of USGS rows) scope with `MonitoringLocation.usgs`. A location is **stale** when `latest_observed_at` is blank or older than 1 week; search/map/directory/nearby drop or demote those tips.
- **`time_series`** — per-parameter series metadata for a location. Unique `provider_series_id` (USGS OGC time-series id today). `measurement_kind ∈ {water_level, discharge, temperature}`, `selected_for_display` gate, `primary_series`. Daily-only providers may have no continuous tip; `HydrographSeries` falls back to daily points and gauge UI hides sub-daily range tabs via `MonitoringLocation#daily_only?` / `#chart_ranges`.
...
- Chart browsers never talk to R2 directly — `/api/gauges/:id/observations` reads R2 dailies for `1y` / `3y` / `Ny` when `DAILY_ARCHIVE_READS=1`.

## 7. Ingestion pipeline

External data flows in through namespaced clients → sync objects → Sidekiq jobs, with rake tasks as manual entrypoints.

- **Clients:** `Usgs::Client` (OGC API, optional `X-Api-Key`, GeoJSON `next` link pagination), `Nwps::Client` (gauge lookup by site number / LID), `Nldi::Client` (public Network Linked Data Index; no API key), `Usbr::Client` (RISE), `Usace::Client` (CWMS), `Cdec::Client`. USGS/NWPS/NLDI/USBR pace requests via `*_REQUEST_PAUSE_MS`. Tip/catalog traffic uses `Usgs::Client.for_tip` (`USGS_API_KEY`); history backfill uses `Usgs::Client.for_history(:continuous|:daily|:peaks)`, which pins `USGS_API_HISTORY_CONTINUOUS_KEY` / `_DAILY_KEY` / `_PEAKS_KEY`. Production **fail-closes** if a purpose key is unset (no tip-key fallback); local/test may fall back to `USGS_API_KEY`.
- **Sync objects (`app/models/*_sync.rb`, `history_ingestion.rb`, `display_series_selection.rb`):**
  - `StationCatalogSync` (weekly / bootstrap) — discover active continuous water-body sites, filter via `Usgs::SiteTypes`, upsert series + latest, select display series, prune inactive, refresh nearby, kick a limited `NetworkStations` NLDI pass and enqueue `NetworkRefreshBatchJob` to drain the rest, warm caches. `StationCatalogSyncLock` prevents overlapping runs (job, `BootstrapStateJob`, rake) so two catalog passes cannot occupy both `sync_worker` threads.
  - `LatestObservationSync` (hourly) — refresh `selected_for_display` series, denormalize location columns, warm caches. Enqueues `IvRepairJob` when a new tip jumps more than `CONTINUOUS_GAP_THRESHOLD` past the previous continuous tip on an anchored series. `LatestObservationSyncLock` prevents overlapping runs (job, `BootstrapStateJob`, rake) so two tip passes cannot occupy both `sync_worker` threads.
  ...
  - `HistoryIngestion` (on-demand/batch) — USGS continuous/daily/peaks for charts; gap-aware. Cold/lazy path uses `1y`; deep `3y` daily only for year-ready USGS stations. Batch/lazy scopes are `MonitoringLocation.usgs`. `HistoryBackfillBatchLock` (30m) and `DailyArchiveExportLock` (8h) prevent overlapping batch/export scans. `IvRepairBatchLock` / `IvRepairScarBatchLock` (15m) do the same for the IV lanes.
  - Curated provider syncs (`UsbrReservoirSync`, `UsaceProjectSync`, `NwpsGaugeSync`, `CdecReservoirSync`) upsert allowlisted rows and refresh tips. Companion `*HistoryBackfillJob` classes write agency daily (or NWPS stageflow) into the same R2 / continuous tables. Do not point these catalogs at USGS `HistoryIngestion`.
  ...
  - **Jobs (`app/jobs`) + schedule (`config/sidekiq.yml` on `worker`):** catalog (Sun 03:00), latest (hourly :03), flood (hourly :30), IV tip repair catch-up (Mon–Sat hourly :35), IV scar repair catch-up (Mon–Sat hourly :50), USGS history backfill batch (Mon–Sat every 10 minutes), NLDI network refresh batch (Monday 06:17 UTC on `sync`), leftover daily drain (every 6 hours), prune (daily 09:15 UTC), USBR/USACE/NWPS/CDEC curated syncs (04:20 / 04:35 / 04:50 / 05:05 UTC). Queues: `default` → `worker`, `sync` → `sync_worker`, `iv_repair` → `iv_repair_worker`, `iv_repair_scar` → `iv_repair_scar_worker`, `backfill` → `historical_worker`.
- **Rate-limit protection:** `ApplicationJob` retries transient `Usgs::Client::Error` but **discards** USGS and NLDI `RateLimitError`s (and `ActiveJob::DeserializationError` for deleted records). Postgres read-only windows raise `DatabaseReadOnlyError` (15 × 2 min) and open `DatabaseReadOnlyCircuit`. `Usgs::RateLimitCircuit` is **per API-key id** (`tip`, `history_continuous`, `history_daily`, `history_peaks`, `history_iv_repair`, `history_iv_repair2`; TTL = rest of the UTC hour) and trips on a real USGS 429. `Nldi::RateLimitCircuit` is a single unauthenticated window (`nldi:rate_limit_circuit`, same rest-of-UTC-hour TTL) tripped by NLDI `OVER_RATE_LIMIT` 429s; navigation is ~400 req/hr per client, then ~1/10s, so `NLDI_REQUEST_PAUSE_MS` defaults to 10000 and Faraday does not retry 429. `NetworkRefreshBatchJob` / `NetworkStations.refresh` skip or stop the pass when that circuit is open. Tip/catalog jobs check the tip circuit; history jobs check `Usgs::HistoryKeyPool.exhausted?` / purpose availability so one purpose trip does not stop the others (or tip sync). We do **not** keep a local used/remaining counter — USGS quota is not accurately mirrorable from our side. History backfill additionally no-ops on Sundays, honors `HistoryBackfillLock` (1h TTL, 6h cooldown), skips when the backfill queue is still busy, and is enqueued lazily from `GaugesController#show` when a USGS station `needs_history_backfill?` (phase-1 `1y` only). The batch sizes phase-1 as absolute `HISTORY_BACKFILL_BATCH` stations (soft-capped by a theoretical 1000/hr planning ceiling) and deep `3y` up to `HISTORY_DEEP_BACKFILL_BATCH` when the daily circuit is clear. Circuit state per key is shown on `/admin`.
...
4. **Cloudflare tag purge** via `Cloudflare::CachePurge` + `EdgeCacheInvalidation` after latest/flood/catalog syncs (and after history ingestion). Requires `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ZONE_ID`; no-ops when unset. National syncs purge aggregate tags (`gauges`, `states`, `home`, `map`, `alerts`, map API tags); state-scoped syncs also purge that state’s gauges. History ingestions **coalesce** tags through `EdgeCachePurgeBuffer` + debounced `EdgeCachePurgeJob` (or `EdgeCacheInvalidation.coalesce` for synchronous `usgs:backfill`) so a multi-station backfill does not fire one Instant Purge per station. `Cloudflare::CachePurge` retries rate-limit responses with backoff. Purge failures are logged/Sentry’d and do **not** fail the sync; `EdgeCacheInvalidation#flush_pending!` **re-adds** drained tags when flush raises so a Cloudflare blip does not drop them.
5. **No Rails session on cacheable pages.** `ApplicationController` sets `request.session_options[:skip] = true` by default so public HTML/JSON does not emit `_waterlevels_session`. Cloudflare treats `Set-Cookie` as `BYPASS` even when a Cache Rule marks the path Eligible for cache. **Contact, subscriptions, and admin** opt into sessions (`PagesController` / `ContactsController` / `SubscriptionsController` / `Subscriptions::*` / `Admin::*` via `enable_session?`) for CSRF + flash / login. Gauge HTML stays sessionless and cacheable; `POST /subscriptions` therefore `skip_forgery_protection` and relies on honeypot + Turnstile + IP rate limit. `csrf_meta_tags` render only when the session is enabled. `PUT /temperature_unit` skips forgery protection (preference cookie is also set client-side). **If you add another form or anything that needs CSRF/flash/session, you must opt that controller into `enable_session?` and keep its path out of the edge cache (or accept that it cannot be CDN-cached).** Do not put CSRF meta tags back on the global layout for cacheable pages. Admin and `/subscriptions*` set `private, no-store`.

**Cloudflare dashboard (ops):** public HTML/JSON need a Cache Rule with **Eligible for cache** + **Origin Cache Control: On**. Bypass `/contact*`, `/subscriptions*`, and `/admin*` (and any future session-backed or credentialed paths). Without that rule, HTML stays `DYNAMIC`/`BYPASS` and tag purge has nothing to invalidate.
...
- **Chart data:** the gauge view passes an observations URL; `hydrograph_controller` fetches `/api/gauges/:id/observations` per measurement. `HydrographSeries` returns `{ kind, label, range, unit, parameter_code, points:[{t,v}], peaks:[…] }`, using continuous points for `24h/7d/30d` (hidden on `daily_only?` reservoirs) and R2 daily points for `1y` / `3y` / `10y`. Long tabs lock per selected measurement when USGS publishes no daily DV (`usgs_daily_absent`). `10y` is gated by `has_10y_history?` (daily older than 37 months).
...
`GET /contact` renders `Contact::FormComponent` with a Turnstile widget (`action=contact`) and is served `private, no-store`. `POST /contact` runs an `invisible_captcha` honeypot, validates a `ContactMessage`, verifies Cloudflare Turnstile (`TurnstileVerification` checks token, **action**, and **hostname**; bypassed in test when the secret is unset), and enqueues `ContactMailer` onto the default Sidekiq queue (`worker` dyno; delivered via bento-actionmailer in production, with premailer-rails inlining CSS). There is no listener on Action Mailer’s built-in `mailers` queue. Alert signup mail (`AlertMailer` verify/manage-link) and `AlertDeliveryJob` use the `notifications` queue / `notifications_worker`. Recipient/from configured via `CONTACT_TO_EMAIL` / `MAIL_FROM`. Hostnames: `TURNSTILE_HOSTNAMES` if set, else `localhost`/`127.0.0.1` in development, else `APP_HOST` plus its `www.` twin.

## 11. Domain/value objects

Encapsulate domain knowledge in small, well-named objects rather than scattering constants: `DataProviders` (agency labels + official-site URLs), `Usgs::ParameterCodes` (codes + preference ranking + kind mapping), `Usgs::StateCodes` (FIPS ↔ USPS), `Usgs::SiteTypes` (water-body allowlist), `Usgs::TimeZones`, `Nwps::FloodCategories`, `UnitLabel` (e.g. `ft³/s`), `NearbyStations` (grid + haversine; live tips only), `TrendComparison` (24h + YoY deltas), `SiteStats`, `PopularWaterways`.
...
- **WebMock** with `disable_net_connect!(allow_localhost: true)`; all USGS/NWPS/USBR/USACE/CDEC/Turnstile calls are stubbed. `TURNSTILE_SECRET` is stripped in setup so the form path is testable.
...
- **Processes:** `Procfile` → `web` (Puma) + `worker` (Sidekiq default queue + scheduler) + `sync_worker` (`sync`, concurrency 2) + `iv_repair_worker` (`iv_repair`) + `iv_repair_scar_worker` (`iv_repair_scar`) + `historical_worker` (`backfill`, concurrency 2) + `notifications_worker` (`notifications`, concurrency 1) + `release` (`db:migrate`); `Procfile.dev` → Rails + JS/CSS watchers. Default / notifications Sidekiq processes stay concurrency 1 for 512MB eco dynos; historical_worker is 2 with batch/export run locks; tip IV repair uses concurrency 4 and scar IV repair concurrency 2; flood sync is one long job with an inter-state timer. Scale `notifications_worker` whenever email alerts ship (`heroku ps:scale notifications_worker=1`) — digest/quiet cron ticks land on that queue.
...
- **Heroku target:** web + worker dynos, Postgres + Redis add-ons; env `USGS_API_KEY`, purpose-pinned history keys, `REDIS_URL`, `DATABASE_URL`, `APP_HOST`; post-deploy `bin/rails usgs:enqueue_bootstrap`. Redis Cloud uses a public CA — `lib/redis_config.rb` defaults to `VERIFY_PEER`. Set `REDIS_SSL_VERIFY=none` only for a self-signed `rediss://` host.
