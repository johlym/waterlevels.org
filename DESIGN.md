...
- **Jobs (`app/jobs`) + schedule (`config/sidekiq.yml` on `worker`):** catalog (Sun 03:00), latest (hourly :03), flood (hourly :30), IV tip repair catch-up (Mon–Sat hourly :35), IV scar repair catch-up (Mon–Sat hourly :50), history backfill batch (Mon–Sat every 10 minutes), leftover daily drain (every 6 hours), prune (daily 09:15 UTC). Queues: `default` → `worker`, `sync` → `sync_worker`, `iv_repair` → `iv_repair_worker`, `iv_repair_scar` → `iv_repair_scar_worker`, `backfill` → `historical_worker`.
...
- **Processes:** `Procfile` → `web` (Puma) + `worker` (Sidekiq default queue + scheduler) + `sync_worker` (`sync`, concurrency 2) + `iv_repair_worker` (`iv_repair`) + `iv_repair_scar_worker` (`iv_repair_scar`) + `historical_worker` (`backfill`) + `release` (`db:migrate`); `Procfile.dev` → Rails + JS/CSS watchers. Default / historical Sidekiq processes stay concurrency 1 for 512MB eco dynos; tip IV repair uses concurrency 4 and scar IV repair concurrency 2; flood sync is one long job with an inter-state timer.
