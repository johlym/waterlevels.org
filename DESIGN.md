...
- **Ops:** `/up` health check; Sidekiq Web mounted at `/sidekiq` **in development only**. Password-gated `/admin` dashboard when `DASHBOARD_PW` is set (HTTP Basic, username `admin`); returns 404 when unset. Not edge-cached (`private, no-store`).
...
- Retention windows in `ContinuousPruneJob` must stay aligned with the ranges `HistoryIngestion` backfills.
