...
- **Jobs (`app/jobs`) + schedule (`config/sidekiq.yml` on `worker`):** catalog (Sun 03:00), latest (hourly :03), flood (hourly :30), IV repair catch-up batch (Mon–Sat hourly :35), history backfill batch (Mon–Sat every 10 minutes), prune (daily). Queues: `default` → `worker`, `sync` → `sync_worker`, `iv_repair` → `iv_repair_worker`, `backfill` → `historical_worker`.
