...
- **Jobs (`app/jobs`) + schedule (`config/sidekiq.yml` on `worker`):** catalog (Sun 03:00), latest (hourly), flood (hourly :20), IV repair batch (Mon–Sat every 10 minutes), history backfill batch (Mon–Sat every 10 minutes, offset), prune (daily). Queues: `default` → `worker`, `sync` → `sync_worker`, `iv_repair` → `iv_repair_worker`, `backfill` → `historical_worker`.
...
- **Processes:** `Procfile` → `web` (Puma) + `worker` (Sidekiq default queue + scheduler) + `sync_worker` (`sync`, concurrency 2) + `iv_repair_worker` (`iv_repair`) + `historical_worker` (`backfill`) + `release` (`db:migrate`); `Procfile.dev` → Rails + JS/CSS watchers. Default / IV-repair / historical Sidekiq processes stay concurrency 1 for 512MB eco dynos; flood sync is one long job with an inter-state timer.
