...
- **Observation tables**, all FK → `time_series`, all upserted on natural keys:
  ...
  - `daily_observations` — **legacy drain only**. New ingest writes dailies to R2 when archive writes are enabled; prune deletes Postgres rows once present in R2.
...
- **Jobs (`app/jobs`) + schedule (`config/sidekiq.yml` on `worker`):** catalog (Sun 03:00), latest (hourly :03), flood (hourly :30), IV tip repair catch-up (Mon–Sat hourly :35), IV scar repair catch-up (Mon–Sat hourly :50), history backfill batch (Mon–Sat every 10 minutes), prune (daily). Queues: `default` → `worker`, `sync` → `sync_worker`, `iv_repair` → `iv_repair_worker`, `iv_repair_scar` → `iv_repair_scar_worker`, `backfill` → `historical_worker`.
