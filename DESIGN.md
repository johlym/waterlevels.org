...
- **Sync objects (`app/models/*_sync.rb`, `history_ingestion.rb`, `display_series_selection.rb`):**
  ...
  - `HistoryIngestion` (on-demand/batch) — fetch continuous/daily/peaks for charts; gap-aware. Cold/lazy path uses `1y`; deep `3y` daily only for year-ready stations. `HistoryBackfillBatchLock` (30m) and `DailyArchiveExportLock` (8h) prevent overlapping batch/export scans. `IvRepairBatchLock` / `IvRepairScarBatchLock` (15m) do the same for the IV lanes.
...
`GET /contact` renders `Contact::FormComponent` with a Turnstile widget and is served `private, no-store`. It is the only public HTML surface that enables the Rails session (CSRF meta tags + flash). `POST /contact` runs an `invisible_captcha` honeypot, validates a `ContactMessage`, verifies Cloudflare Turnstile (`TurnstileVerification`; bypassed in test when the secret is unset), and enqueues `ContactMailer` onto the default Sidekiq queue (`worker` dyno; delivered via bento-actionmailer in production, with premailer-rails inlining CSS). There is no listener on Action Mailer’s built-in `mailers` queue. Alert signup mail (`AlertMailer` verify/manage-link) and `AlertDeliveryJob` use the `notifications` queue / `notifications_worker`. Recipient/from configured via `CONTACT_TO_EMAIL` / `MAIL_FROM`.
...
- **Heroku target:** web + worker dynos, Postgres + Redis add-ons; env `USGS_API_KEY`, `REDIS_URL`, `DATABASE_URL`, `APP_HOST`; post-deploy `bin/rails usgs:enqueue_bootstrap`. Redis Cloud uses a public CA — `lib/redis_config.rb` defaults to `VERIFY_PEER`. Set `REDIS_SSL_VERIFY=none` only for a self-signed `rediss://` host.
