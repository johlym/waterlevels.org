...
- Optional ops dashboard at `/admin` when `DASHBOARD_PW` is set (HTTP Basic username `admin`). Returns 404 when the env var is unset. Password attempts are rate-limited (Rails `rate_limit`, 10 per 3 minutes per IP). Sidekiq Web is at `/admin/sidekiq` behind the same password.
