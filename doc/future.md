...
**Non-USGS daily 10y (mechanics proving ground):** seven allowlisted reservoirs write year shards the same way USGS dailies do. Backfill still requests 10 years and keeps whatever the agency POR includes. The `10y` tab unlocks once daily coverage extends past 3 years (not only when a full decade is present), so a shorter record still gets the longer chart. USGS `HistoryIngestion` stays at `DAILY_RETENTION = 3.years`. NWPS primary gauges cannot join this path — the official NWPS API keeps only ~30 days of stageflow plus crest/low-water history, not a continuous archive.
