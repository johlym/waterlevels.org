...
- Production gauge signup and manage-link request require `TURNSTILE_SECRET`. `TurnstileVerification` checks widget **action** (`email-notifications` / `manage-link`, plus one-day legacy names) and **hostname** (`APP_HOST` ± `www.`, or `TURNSTILE_HOSTNAMES`). Timezone is JS-detected (hidden on the gauge form). Cap: `ALERTS_MAX_WATCHES` (default 25). Gauge `POST /subscriptions` skips CSRF because the gauge page is sessionless.
