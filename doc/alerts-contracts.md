...
Shipped with #242 (schema, signup, evaluation, digests), #247 (`notifications_worker`), #258 (batched evaluation), #265 (self-contained mail), and #267 (digest links / anti-autolink). Operator runbook: [`README.md`](../README.md#email-alerts-operator). Tip SLA: [`alerts-freshness-sla.md`](alerts-freshness-sla.md).
...
Registry: `app/models/alert_rule_kinds.rb`. New watches get flood + digest via `StationWatch#ensure_default_rules!`. Threshold is created **disabled** from the manage UI unless the subscriber opts in.
...
| `rate_of_rise` | `Alerts::RateOfRiseEvaluator` | No | Calls stub mailer (`{label} alert (preview)`). Do not enable. |
| `in_range` | `Alerts::InRangeEvaluator` | No | Calls stub mailer. Do not enable. |
| `quiet_station` | Event from `AlertQuietScanJob` | No | **Do not enable** — `AlertEvaluationJob` enqueues `quiet_station`, but `AlertDeliveryJob` has no case and raises |
...
404 when `AlertsConfig.disabled?`. Session-backed except the gauge-page POST (honeypot + Turnstile + 20/hr IP; CSRF skipped because the gauge page is cookieless).
...
## Email rendering (as-built)

HTML mail uses `layouts/mailer.html.erb` only — embedded CSS, no external stylesheets/fonts/images/CDNs. Tests in `test/support/mailer_html_assertions.rb` guard that.

| Constraint | Why |
| ---------- | --- |
| Bento Liquid `disable_open` / `disable_click` / `disable_utms` in a hidden `.bento-flags` div | Bento still injects `kodama.*` open.gif when `transactional=true`; iOS Mail then prompts for remote content. Flags must **not** live in an HTML comment — Bento echoes `-->` into the body. |
| `ApplicationMailer` disables view filename annotations around each send | Multi-line `<!-- BEGIN ... -->` annotations also leak a visible `-->` in some clients. |
| `DigestBuilder` includes `url` (canonical `gauge_url`) | Station names in digest / flood / threshold HTML link to the gauge page. |
| `MailerHelper#email_site_number` inserts zero-width spaces every 4 digits; layout sets `format-detection: telephone=no` | iOS Mail / Gmail otherwise turn USGS site numbers into `tel:` links. |
| Digest row order | gage height, then discharge, then flood category. |
| `MailerHelper#email_unit` / `UnitLabel` | `ft3/s` and `ft^3/s` render as `ft³/s`. |

`AlertMailer.deliver_later_queue_name` is `:notifications` so confirmation / manage-link mail stays on `notifications_worker` instead of the unused `mailers` queue.
